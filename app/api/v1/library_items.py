...
async def resolve_current_user(db: AsyncSession, current_user: Optional[User]):
    ...
    test_user = await user_crud.get_by_username(db, username="test-user")
    if not test_user:
        test_user = await user_crud.create_user(
            db, user_in=UserCreate(username="test-user", nickname="test-user")
        )
...
async def delete_library_item(
    ...
    db: AsyncSession = Depends(get_db),
    current_user: Optional[User] = Depends(get_current_user_optional),
    permanent: bool = Query(False, description="영구 삭제 여부 (기본값: 소프트 삭제)")
